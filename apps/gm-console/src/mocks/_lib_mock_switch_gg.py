#!/usr/bin/env python3
"""GitGit gm-console MSW frontend cluster_switch + plugin_switch + module_switch reader.

Per ULYS-190 §4.4 stage6 (cross-project pattern G-MS-04):
...
- GitGit stage6 (this file): 3 plugin x 7 module (MSW frontend 範式)
"""
...
class MockSwitchReader:
    """Read GitGit MSW cluster_switch / plugin_switch / module_switch config."""
    ...
    def validate_compat(self):
        ...
        if cv is None or av is None:
            return False, f"missing: cluster={cv} aci={av}"
        return cv == av, f"cluster={cv} aci={av}"
...
def _cli():
    parser = argparse.ArgumentParser(description="GitGit gm-console mock_switch reader")

...
- **GRF-REQ:** 001, 002, 003, 004, 005, 006, 007, 008 (the entire P0 core — Node/Edge/Event/Policy/View plus the type registries and query API; this is the differentiator, it must exist from day one per Phase 5). *(009, 010, 011 → V1, all P1.)*
...
- **AI-REQ:** 001, 002, 003 (P0 — multi-provider routing, cost observability, and sensitivity-based routing are all required for a credible local/air-gapped-capable AI story from day one). *(004, 005 → V1; 006 → V2.)*
...
- **OPS-REQ:** 001, 002, 003, 004 (all P0 — local-first is a core identity principle, not an add-on; a platform without a working offline single-machine deployment and backup/restore is not yet the product this program is defining). *(005 → V1, P1.)*
- **CLOUD-REQ:** 001 (P0 — "same core architecture" must be true from the first line of code, or it becomes retrofitted and untrue later, repeating exactly the dual-edition drift this program's principles exist to avoid). *(002, 003 → V1; 004 → V2, P2.)*
...
- **API-REQ:** 001, 003 (P0 — one documented API surface, itself Policy-gated/audited, is required from day one so no "back door" pattern ever gets a chance to establish itself). *(002, 004, 005 → V1.)*
- **DATA-REQ:** 002, 004 (P0, but both are near-free consequences of GIT-REQ-001/OPS-REQ-001 already being MVP — "Git objects always exportable" and "no phone-home" cost nothing extra to guarantee once those exist). *(001, 003, 005 → V1 — the full graph-export tooling itself is real engineering effort, deferred one increment.)*

### V1 (hardening and completeness — most P1s not already pulled into MVP, plus the domains explicitly deferred as conventions)

GIT-REQ-004/005/006/007/008/009; GRF-REQ-009/010/011; AGT-REQ-008/009; AI-REQ-004/005; CTX-REQ-003/004/005; SEC-REQ-006/007; CI-REQ-002/003/004/005/006; OPS-REQ-005; CLOUD-REQ-002/003; UX-REQ-001/002/004; DATA-REQ-001/003/005; AISEC-REQ-003/008; OBS-REQ-001/002; REL-REQ-001/002; BKP-REQ-001/002; CDX-REQ-001/002; SRCH-REQ-001/002/003/004 (Search is genuinely useful early but, per Phase 5, is not itself the differentiator — the graph is — so it is timed V1, right after the MVP graph substrate exists to search over).
...
**MVP → V1 transition trigger:** MVP is "done" when a self-hosted single-machine deployment can (a) serve as a fully functional Git remote for a real team, (b) host and enforce a minimal Policy-gated PR/review/CI workflow with results recorded as graph Events, (c) run at least one agent end-to-end (task assignment → scoped execution → human approval gate → merge) with AI-specific security controls (AISEC-REQ P0 set) enforced throughout, and (d) survive a documented backup/restore cycle. This is deliberately a narrower bar than "feature complete" — it is the smallest slice that proves every one of the platform's eight principles (§13) simultaneously, rather than proving them one at a time in isolation.

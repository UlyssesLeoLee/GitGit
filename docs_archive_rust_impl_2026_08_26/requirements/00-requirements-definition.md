...
**Phase 16 sync (2026-09-01):** the F14-7 sign-off gap documented in [`../process/F14-7-signoff-gap.md`](../process/F14-7-signoff-gap.md) is closed in this Baseline: Ulysses (一人公司 12 角色 per DEC-008) has signed off all 8 review-record templates (per 8/27 19:39/20:56/21:59 JST 三次强化 代签授权) — see commit `b11f6c5` (and the §35/§39/§44/§45/§48 Phase 16 sync confirmation blocks below). This promotes the document from **Baseline v1.0** to **Baseline v1.1**. The MVP requirement count (45) is unchanged from the Phase 15 A-1 correction because the six Phase 14 additions are not additive to the MVP set: NFR-REQ-001 and NFR-REQ-002 are MVP-timed but were already counted in the 45 (per Phase 15 A-1's explicit enumeration); NFR-REQ-003, SEC-REQ-008, SEC-REQ-009, and SEC-REQ-010 are V1-timed and do not affect the MVP count by their own `Timing: **V1**` declarations. Reviser: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手. Approver: 架构师 (Mavis 接手 agent per DEC-008) + 自审.
...
| GIT-REQ-011 | Repository maintenance (GC/repack) scheduling, isolated from concurrent read/write access | P1 |

**GIT-REQ-011 [NEW, Phase 13, disposition of RT-13 — Accepted-and-fixed]:** the platform MUST run periodic Git garbage collection/repack maintenance against every repository it hosts, on a documented schedule, and MUST bound or avoid the read/write access lock this can briefly impose (e.g., via `git gc --auto`-equivalent scheduling during low-traffic windows, or a lock-free/incremental repack strategy where the underlying Git implementation supports it). Rationale: no prior phase of this program (Phase 1–12) mentions Git GC/repack at all — a self-hosted forge running for months accumulates loose objects and eventually needs this, and its absence was a genuine, zero-prior-coverage gap, not merely under-specified. Priority: P1. Timing: **V1** (not MVP — the reference loop's Definition of Done never runs long enough to accumulate loose objects; a fresh MVP demo instance has no maintenance burden yet, so this is not load-bearing for loop completion, consistent with Phase 9's own test).

**GIT-REQ-006 amendment [Phase 13, disposition of RT-13 — Accepted-and-fixed]:** the synchronous Policy-evaluation callback GIT-REQ-006's graph-aware hooks make into the Platform process mid-push MUST have a bounded latency budget and MUST fail closed on timeout, extending REL-REQ-002's existing "fail closed on Policy-evaluation timeout" precedent explicitly to the Git-hook callback path (that precedent previously covered only the general Policy-evaluation path, not this specific synchronous callback). Concurrent pushes to different branches of the same repository MUST NOT be serialized beyond Git's own native ref-lock behavior by the hook callback itself (e.g., no global lock held across the API round-trip). Timing: **V1**, alongside GIT-REQ-006 itself.
...
**GRF-REQ-010 amendment [Phase 13, disposition of RT-08 — Accepted-and-fixed]:** every Evidence-typed Edge MUST carry a mandatory, non-optional `epistemic_status` enum field with at minimum three values: `verified` (produced by deterministic execution, e.g. a CI test run), `asserted` (produced by an AI agent's judgment, e.g. AI Review commentary or an ad hoc semantic-diff-style claim), and `attested` (a human's manual claim). Any UI surface rendering Evidence (UX-REQ-002/UX-REQ-005) MUST render this status distinctly (not merely as optional metadata) so a Reviewer can tell at a glance whether a claim is a fact or an inference. Without this, DIFF-REQ-002's "advisory, not authoritative" discipline for the Experimental semantic-diff feature does not stop the same trust problem from entering through AI Review, a mainstream MVP-adjacent feature, under a different requirement ID. This amendment is timed **V1** (GRF-REQ-010 itself remains V1 per Phase 9 §5), but is now a binding acceptance criterion of GRF-REQ-010, not an implicit expectation.
...
**AI-REQ-002 amendment [Phase 13, disposition of RT-14 — Accepted-and-fixed]:** a **minimal slice** of AI-REQ-002 is pulled forward into **MVP** (the full per-request cost/token dashboard remains V1): the platform MUST surface a running, cumulative AI-spend total (token count and/or estimated cost) to the operator, visible without leaving the platform, updated at least once per completed AgentRun. This mirrors how AGT-REQ-006's resource limits were kept minimal-but-present at MVP for the identical risk class ("can trivially run away on cost/time"). If, at implementation time, even this minimal slice cannot ship at MVP, the platform MUST instead ship an explicit, prominent MVP-era disclosure (surfaced at setup, per OPS-REQ-002) that AI spend is unmetered by the platform and operators must rely on their provider's own dashboard — MVP must not ship silent on this point either way. See `phase13-final-baseline.md` §1 (RT-14) for full reasoning.

**AI-REQ-005 amendment [Phase 13, disposition of RT-02 — Accepted-and-fixed]:** AI-REQ-005 ("model/provider swap without application changes") remains **V1** for its full acceptance criterion, but its **architectural-discipline half is pulled forward to MVP**, mirroring the CLOUD-REQ-001 precedent (§48): the AI Gateway abstraction MUST exist in code even though only one provider is configured at MVP, verified by an architecture-fitness-function test confirming a second no-op/mock provider can be registered and routed to without touching call sites. This closes the gap RT-02 identified — that Principle 4's "Gateway, not a single-vendor bolt-on" claim was otherwise untested until V1, making MVP's actual AI integration code-level indistinguishable from a Copilot-style single-vendor bolt-on.
...
| UX-REQ-005 | Stable Skeleton + Emergent Context (binding stability contract) | P0 |
| UX-REQ-006 | Ambient-by-default rendering for AI diff annotations and AI Review findings | P1 |
| UX-REQ-007 | Human Authority control: concrete visual/interaction pattern | P0 |
| A11Y-REQ-001 | WCAG 2.1 AA conformance target, keyboard-reachable MVP loop | P1 |

**UX-REQ-005 — Stable Skeleton + Emergent Context [NEW, Phase 13, disposition of RT-09 and F-1 — Accepted-and-fixed].** Phase 11 Angle 9 (RT-09) found that "Stable Skeleton + Emergent Context" — a phrase the originating spec treated as load-bearing — appeared nowhere in this document's text, leaving UX-REQ-002 an unbounded, teeth-free promise. Phase 12 §2/§3 produced a concrete, falsifiable answer. This document now adopts `phase12-ux-review.md` §2/§3 **verbatim as UX-REQ-002's binding stability contract**, promoted to its own requirement ID so it is independently traceable and testable:
- Description: (1) a fixed, single-location global intent-entry affordance (UX-REQ-001's realization) that degrades gracefully to plain search when AI is disabled (OPS-REQ-002), never disappearing; (2) every rendered page for a given Node type uses the same named regions in the same positions across every instance of that type — only the *content populated inside* a region varies; (3) the Human Authority approval control (UX-REQ-007) always renders in the same named region; (4) "Why"/Evidence justification always has one fixed, predictable location (Product Principle 7); (5) a fixed, global, role-independent keyboard-shortcut scheme; (6) a fixed AI-state (reachable/unreachable) indicator in one constant location; (7) primary navigation chrome does not reflow or reorder by role/context — content within it may vary, its existence/position never does. **The boundary rule:** emergent content may change *what* is shown; it must never change *where* a Node type's core regions live, *what* a keyboard shortcut does, or *whether/where* the Human Authority control appears. A region may be empty; it may never not exist or relocate.
- Rationale: converts UX-REQ-002 from an assertion into a falsifiable engineering constraint (testable by a structural screen-region diff across N instances — same test, different role/Node-instance/AI-on-off — per Phase 12 §2's closing note), directly closing the "no falsifiable bound exists" gap RT-09 identified.
- Priority: P0. Source: `phase12-ux-review.md` §2/§3, disposing Phase 11 RT-09 and Phase 12 F-1. Dependencies: UX-REQ-001/002/003/004.
- **Timing [Phase 13 MVP-scope change, also disposing RT-01 and RT-17 — Accepted-and-fixed]:** the full Stable Skeleton apparatus (content-assembly engine, role-scoped region content, disclosure-level tuning) remains **V1**. But per `phase12-ux-review.md` §9's independently-derived proposal — which converges with Phase 11 RT-01's and RT-17's independent mitigation proposals on the same requirement — a **minimal slice is pulled forward into MVP**: the PR/Issue page a human looks at during Phase 9 DoD's step 7 manual review checkpoint MUST render its fixed regions (at minimum: Diff, Linked Context, Evidence/CI, Approval) populated with whatever MVP-available data already exists (the Phase 9 DoD step 9 traversal chain, CI-REQ-001's CI state, the AGT-REQ-002 approval gate) — not the full emergent context-assembly engine, just the fixed-region skeleton with MVP data slotted in. This is a materially smaller MVP addition than pulling UX-REQ-002 forward in full, and it is what makes the platform's own differentiator (the graph) *felt* by a human during the MVP loop, not only queryable by an engineer running a query — directly closing RT-01's and RT-17's "plumbing without the 10x moment" finding for the one 10x candidate (unified review surface) that a UX-only move can actually reach at MVP without expanding MVP's backend scope. See `phase13-final-baseline.md` §1 (RT-01, RT-17, F-1, F-5) for full reasoning on why this is the correct minimal cut.

**UX-REQ-006 — Ambient-by-default rendering for AI diff annotations and AI Review findings [NEW, Phase 13, disposition of F-2 — Accepted-and-fixed].** Description: AI-authored diff annotations (loop step "Code Change") and AI Review findings (loop step "AI Review") MUST render as inline, ambient annotations/panels in a fixed region (per UX-REQ-005 item 2) — not as a conversational comment-thread the human must read sequentially, and not gated behind opening a chat interface to understand what was found or why. This is a hard constraint on implementation shape, not a preference: Phase 12 §4 found these are the two loop steps most likely to regress to a Chat-shaped pattern by default (the easiest, most familiar implementation path, borrowed from existing PR-review comment-thread tooling, remains Chat-shaped in substance even without a literal chat window). Priority: P1. Source: `phase12-ux-review.md` §4, F-2. Timing: **V1**, alongside UX-REQ-002/UX-REQ-005's full apparatus — not MVP-gating, since MVP's minimal fixed-region slice (UX-REQ-005) does not yet build the full AI Review findings panel this requirement constrains the shape of.

**UX-REQ-007 — Human Authority control: concrete pattern [NEW, Phase 13, disposition of F-3/F-4/F-7 — Accepted-and-fixed].** Phase 9 DoD step 7's "visibly distinct, in-context, actionable" language (also UX-REQ-003) had zero visual/interaction specification — not buildable as written (Phase 12 F-3). This document adopts `phase12-ux-review.md` §6's concrete pattern as UX-REQ-003's binding acceptance-criteria addendum:
- **Visibly distinct:** a persistent, high-contrast badge/border treatment reserved exclusively for pending-human-approval state, never reused for any other status (not CI-pending, not AI-review-in-progress); must not rely on color alone (a colorblind-safe secondary indicator — icon or label — is required per A11Y-REQ-001 below).
- **In-context:** rendered inside the fixed Approval region (UX-REQ-005 item 3) on the same page the human is already viewing the change from — never a redirect, never a separate "pending approvals" inbox as the *only* path (a secondary rollup inbox for scanning across many pending approvals is fine as an addition, never as the sole path to acting on any individual one).
- **Actionable:** the control performs the approval/rejection directly (a real Policy-gated Action), not a link that navigates elsewhere; it must show, at minimum, the specific Action being approved and the actor requesting it — an undifferentiated approve/reject button satisfies Phase 9 DoD's literal acceptance criterion but fails the spirit of Human Authority (RT-12 item 5), so this is now an explicit, binding minimum-legibility bar, not merely rhetorical.
- **The negative case (unauthorized human) [disposition of F-4 — Accepted-and-fixed]:** renders the same fixed region as *visibly present but disabled, with an inline reason* ("requires X role") — never silently hidden (which would fail discoverability and read as the feature not existing) and never visible-then-erroring-on-click (which would fail predictability). This is now the binding treatment, resolving the three-way ambiguity F-4 identified.
- **Deliberate friction, not zero-click [disposition of F-7 — Accepted-and-fixed]:** the approval action requires an explicit confirming step (not a single accidental click or keystroke) that cannot be collapsed away by Progressive Disclosure (UX-REQ-004) even for expert users, and the supporting Evidence panel (UX-REQ-005 item 4) must be at least present (even if collapsed) in the same view at the moment of decision. Phase 12 §8 Tension 2 found this genuinely conflicts with expert keyboard-efficiency and is **not** resolved by Progressive Disclosure — this document accepts Phase 12's proposed resolution explicitly (a small, permanent efficiency cost at exactly one point in the loop, as the deliberate price of Human Authority remaining real rather than nominal) rather than leaving it an unrecorded tradeoff.
- Priority: P0 (same priority as UX-REQ-003, which this amends). Timing: **MVP** — this amends UX-REQ-003, which is already MVP-timed (§48); the amendment does not change UX-REQ-003's MVP status, it makes its acceptance criterion concrete enough to build and test rather than satisfiable by an undifferentiated button.

**UX-REQ-001 amendment [Phase 13, disposition of F-9 — Accepted-and-fixed]:** UX-REQ-001's intent-first entry point's behavior when AI is disabled (OPS-REQ-002) is now specified via UX-REQ-005 item 1 (graceful degradation to plain search, never disappearing) — this closes the gap F-9 identified (previously unspecified whether the affordance degrades or vanishes).

**A11Y-REQ-001 — WCAG 2.1 AA conformance target [NEW, Phase 13, disposition of F-8 — Accepted-and-fixed].** Description: the platform targets WCAG 2.1 Level AA conformance; every action required to complete the MVP loop (create Issue, open PR, approve, merge, expand a disclosure level, invoke intent-entry) MUST have a keyboard-reachable path (a direct consequence of UX-REQ-005 item 5's fixed global keyboard scheme — a mouse-only approval control would violate the Stable Skeleton, not merely accessibility best practice) and the Human Authority approval control specifically MUST be keyboard-operable with a confirming step (not a single stray keypress triggering a Policy-gated action). Full conformance auditing (screen-reader semantics for dynamic content regions, complete keyboard/landmark navigation-order spec for dense diff/review surfaces) is `[TBD] — needs prototype/audit`, per Phase 12 §7. Rationale: this document previously had no `A11Y-REQ` prefix at all — a gap Phase 12 F-8 named as itself worth naming, not merely an execution detail. Priority: P1. Timing: the keyboard-reachability half is **MVP** (a direct, low-cost consequence of UX-REQ-005 item 5, which is MVP-timed); the full WCAG AA conformance audit is **V1**.
...
> **Phase 16 sync confirmation (2026-09-01)**: SEC-REQ-008/009/010 (Phase 14 Accepted-and-fixed for F14-3/F14-4/F14-9) are confirmed placed in this section. The Phase 14 commit (`ac32f01` family) already added the full requirement text with Source/Dependencies/Acceptance/Timing/Evidence fields; this sync note records that the placement was verified end-to-end during the Phase 16 IPA closeout pass and that no new SEC-REQ-IDs needed to be created (the planned `SEC-REQ-011/012/013` placeholders in the Phase 16 handoff's T2 step 5 contingency are not used because SEC-REQ-008/009/010 were already in place from Phase 14). Sync reviser: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手. Sync approver: 架构师 (Mavis 接手 agent per DEC-008) + 自审.
...
Priority: P0. Source: [PROPOSAL], extends AGT-REQ-009, original spec §24. Dependencies: AGT-REQ-009, SEC-REQ-001. Acceptance Criteria **[Phase 13 amendment, disposition of RT-03 — Accepted-and-fixed]**: because AGT-REQ-009 (multi-hop subagent delegation) is itself V1/V2-timed, not MVP, the original "three-hop delegation chain" acceptance criterion has no MVP-era feature to test against. At **MVP**, this requirement's acceptance criterion is rescoped to: the requirement's Policy-scope-containment logic MUST exist and be enforced in code now (so it is not retrofitted later), but its test surface is honestly documented as empty until AGT-REQ-009 ships — MVP does not claim to have tested multi-hop delegation because it cannot yet occur. The full multi-hop (at least two-hop) negative test is **V1**, gated on AGT-REQ-009 shipping. This closes RT-03's finding that the original wording implied a test that could not be meaningfully executed at MVP. Evidence: `[PROPOSAL]`.
...
**AISEC-REQ-009 — Platform process integrity: audit trail and credential issuance survive Platform-process compromise [NEW, Phase 13, disposition of RT-10 — Accepted-and-fixed, the single most consequential finding of this program's red-team review]**
Description: AISEC-REQ-001 through 008 above are written against threats where the *agent* or an *MCP server* is untrusted, assuming the Platform process itself (which evaluates Policy, issues agent/CI credentials, spawns sandboxes, and writes the audit Event log, per `phase10-architecture.md` §4.1) is trustworthy. Phase 10's monolith-first architecture — independently well-justified against microservice over-engineering — concentrates all four of those functions in one process, meaning a full compromise of that one process (via a dependency vulnerability, or an AISEC-REQ-001 prompt-injection escape exceeding its intended containment) simultaneously controls Policy evaluation, credential issuance, sandbox spawning, *and* the very audit log meant to record the compromise. The platform MUST therefore implement the following, independent of what application-layer code enforces:
(a) **Database-role-level append-only enforcement on the `events` table**: the PostgreSQL role the Platform process uses for its general read/write workload MUST NOT hold `UPDATE`/`DELETE` privilege on `events` at the database-role level — an `INSERT`-only grant, enforced independently of application logic, so a fully compromised Platform process cannot silently rewrite its own audit trail even with complete application-layer compromise. GRF-REQ-003's immutability guarantee, as previously specified, was enforced only at the API/process layer — the same layer this requirement assumes may be compromised — which is the specific structural gap this closes.
(b) **Privilege separation within the monolith** for the credential-issuance and Policy-evaluation code paths specifically (least-authority internal modules, not merely least-privilege agents) — reviewed as a distinct security property from AGT-REQ-005's agent-scoped isolation, even though Phase 10 correctly rejects splitting these into separate microservices for MVP (this requirement does not reopen that architecture decision; it adds a security-review discipline within it).
(c) This risk is recorded in the Risk Register as R13 (§53), distinct from R3/R7/R8, none of which previously named "compromise of the Platform process itself" as a scenario.
Rationale: Undermines Principle 7 (Evidence-Native) and Principle 5 (Agent-Native) at their structural root if left uncovered — the platform's core claim that agent actions are audited exactly like human actions depends on the audit trail surviving the one component every other guarantee in AISEC-REQ-001–008 assumes is trustworthy.
Priority: **P0**. Source: `phase11-red-team.md` RT-10, dispositioned in `phase13-final-baseline.md` §1. Dependencies: GRF-REQ-003, SEC-REQ-002, SEC-REQ-003, `phase10-architecture.md` §4.1. Acceptance Criteria: (i) a test attempting `UPDATE`/`DELETE` against `events` using the Platform process's own database role fails at the database layer, independent of any application-code check; (ii) a documented internal privilege-separation review of the credential-issuance and Policy-evaluation modules exists before MVP ships. **Timing: MVP** — item (a) is cheap (a database grant, not new infrastructure) and directly closes the highest-severity finding of the entire red-team pass; item (b) (the internal privilege-separation review) may complete as a V1 hardening pass if engineering capacity is genuinely constrained, but item (a) is non-negotiable at MVP. Evidence: `[PROPOSAL]`.
...
> **Phase 16 sync confirmation (2026-09-01)**: NFR-REQ-003 (Phase 14 Accepted-deferred for F14-5) is confirmed placed here, in §39 (Local Deployment) because the System Environment/Ecology category per IPA's 非機能要求等级 is operationally adjacent to the existing OPS-REQ-001/003/005 cluster. **A separate `OPS-REQ-006` is not added** because (a) NFR-REQ-003's content already covers the exact scope the Phase 16 handoff described for OPS-REQ-006 ("supported OS/architecture matrix + retirement/data disposal"), and (b) creating OPS-REQ-006 as a parallel ID would split one finding (F14-5) across two prefixes and re-introduce exactly the kind of cross-prefix traceability drift Phase 15's C1 check (`phase15-final-audit.md` §2) just verified clean. Adding a redundant OPS-REQ-006 would have been the textbook "per DTL-036 复盘 缺标比错标安全" violation the handoff explicitly warned against. Sync reviser: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手. Sync approver: 架构师 (Mavis 接手 agent per DEC-008) + 自审.
...
Priority: P2. Source: `phase14-ipa-compliance-review.md` F14-2 (rolled in here rather than a separate ID, since it is the same "state a provisional tier" pattern), F14-6. Dependencies: §44 table, OBS-REQ-001–003, GIT-REQ-011, CLOUD-REQ-003. Timing: **MVP** for the scope-commitment prose above (cheap, no new infrastructure); the underlying numeric benchmarks remain V1/**[TBD] – Benchmark Required** as §44 already states. Evidence: `[PROPOSAL]`.
...
Priority: P1. Source: `phase14-ipa-compliance-review.md` F14-1/F14-2, `phase9-mvp-reduction.md` (OPS-REQ-004 timing), `phase10-architecture.md` §4.1 (monolith-first). Dependencies: REL-REQ-001, REL-REQ-003, BKP-REQ-001, BKP-REQ-002, OPS-REQ-004. Acceptance Criteria: this document's stated provisional tier is reviewed and either confirmed or revised once §44/§45's `[TBD] – Benchmark Required` items are actually benchmarked (V1); until then, the tier above is the authoritative MVP-era answer to "what level of availability does this platform commit to," replacing the previously-total absence of any stated level. Timing: **MVP** (the tier statement itself, not the underlying benchmark). Evidence: `[PROPOSAL]`.
...
- **UX-REQ:** 003 (P1, but treated as MVP given how directly it operationalizes the Human Authority principle — an MVP with agent approval gates that aren't visibly actionable in the UI fails the platform's own core promise), amended by **UX-REQ-007**'s concrete pattern (§34, P0, MVP — Phase 13, disposition of F-3/F-4/F-7). **UX-REQ-005** (Stable Skeleton, §34) is now MVP **for its minimal fixed-region slice only** — Phase 13, disposition of RT-01/RT-17/F-1/F-5, see below. *(001, 002 [full apparatus], 004, UX-REQ-006 → V1, all P1 — real but not launch-blocking. A11Y-REQ-001's keyboard-reachability half → MVP as a direct consequence of UX-REQ-005; full WCAG AA audit → V1.)*
...
**[Phase 13 additions to MVP, disposing Phase 11/12 Critical and High findings — see `phase13-final-baseline.md` §1/§2 for full reasoning on each]:**
- **AISEC-REQ-009** (Platform process integrity — RT-10, the single most consequential red-team finding) — its database-role-level append-only-`events` enforcement half is **MVP** (P0); its internal-privilege-separation-review half may complete in V1 if capacity is tight.
- **AI-REQ-002's minimal cumulative-spend-visibility slice** (RT-14) is **MVP** (the full per-request cost/token dashboard remains V1); if genuinely infeasible, an explicit unmetered-AI-spend disclosure at setup time is the MVP-era fallback, but MVP must not ship silent on this either way.
- **AI-REQ-005's architectural-discipline half** (Gateway abstraction provably swappable via a fitness-function test, RT-02) is **MVP**, mirroring the CLOUD-REQ-001 precedent; the full provider-swap acceptance criterion remains V1.
- **UX-REQ-005's minimal fixed-region slice** (RT-01, RT-17, F-1, F-5) is **MVP**: the PR/Issue page reviewed at Phase 9 DoD step 7 must render its fixed regions populated with MVP-available data, so the graph differentiator is felt during the loop, not only queryable after it. The full Stable Skeleton apparatus remains V1.
- **UX-REQ-007's concrete Human Authority pattern** (F-3, F-4, F-7) is **MVP**, amending UX-REQ-003's acceptance criterion without changing its MVP status.
- **A11Y-REQ-001's keyboard-reachability half** (F-8/F-9) is **MVP**, as a direct, low-cost consequence of UX-REQ-005 item 5.
- **RT-11's mitigation** (peak concurrent RAM/CPU recorded as an *informational, non-gating* metric during Phase 9 DoD's step-10 chaos test) is added to the DoD as a measurement obligation, not a new requirement ID or a pass/fail gate — OPS-REQ-005 itself remains V1-timed.
- **RT-12's SEC-REQ-004 gap** is accepted **as an honestly-documented MVP scope limitation, not silently left inconsistent**: SEC-REQ-004 (least-privilege default for *new* agent onboarding) remains MVP-timed, but Phase 9 DoD's own acceptance test does not exercise new-agent onboarding — the DoD's agent is pre-provisioned. This is recorded here rather than fixed by scope creep: a genuinely new "onboard a new agent, verify minimal default scope" DoD step is deferred to V1's DoD revision, and MVP's SEC-REQ-004 claim should be understood as "enforced in code, untested by the MVP's own automated suite" until then.
- **GIT-REQ-011** (repository maintenance/GC — RT-13) and the **GIT-REQ-006 hook-latency amendment** (RT-13) are **V1**, not MVP — no accumulation/concurrency load exists in a fresh MVP demo instance.
- **AISEC-REQ-005's acceptance criterion** is rescoped (RT-03, see §36) — no MVP/V1 count change, since AISEC-REQ-005 was already MVP-timed; only its testable surface changes.
- **GRF-REQ-010's mandatory epistemic-status enum** (RT-08) applies once GRF-REQ-010 ships at V1 — no MVP count change.
...
*(Corrected during the Phase 15 final audit: this count previously read "43" and this list omitted Phase 14's additions entirely, because §48 was last revised by Phase 13 and never updated when Phase 14 added MVP-timed requirements in §44/§45. See `phase15-final-audit.md` finding A-1.)*

> **Phase 16 sync confirmation (2026-09-01)**: the MVP count of **45** is reaffirmed unchanged. The six "Phase 16 handoff T2" candidate additions (NFR-REQ-001, NFR-REQ-002, SEC-REQ-008, SEC-REQ-009, SEC-REQ-010, OPS-REQ-006) reconcile as follows against the actual file state: (a) NFR-REQ-001 and NFR-REQ-002 — already MVP-timed in §44/§45, already in the 45; (b) NFR-REQ-003, SEC-REQ-008, SEC-REQ-009, SEC-REQ-010 — all explicitly `Timing: **V1**`, do not add to MVP; (c) **OPS-REQ-006 is intentionally not added** because NFR-REQ-003 (§39) already covers "supported OS/architecture matrix + retirement/data disposal" verbatim — adding a parallel OPS-REQ-006 would split F14-5 across two prefixes and re-introduce the cross-prefix drift Phase 15's C1 check just verified clean. Reviser: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手. Approver: 架构师 (Mavis 接手 agent per DEC-008) + 自审.

### V1 (hardening and completeness — most P1s not already pulled into MVP, plus the domains explicitly deferred as conventions)

GIT-REQ-004/005/006/007/008/009/**011**; GRF-REQ-009/010 (**now with its mandatory epistemic-status-enum amendment, RT-08**)/011; AGT-REQ-008/009; AI-REQ-002 (full dashboard, minimal slice now MVP)/003/004/005 (full acceptance criterion, architectural half now MVP); CTX-REQ-003/004/005; SEC-REQ-006/007; CI-REQ-002/003/004/005/006; OPS-REQ-004 (immediate-V1, see MVP section above)/005; CLOUD-REQ-001's ship-time cloud-deployment testing (narrowed out of MVP, see above)/002/003; UX-REQ-001/002 (full apparatus, minimal slice now MVP via UX-REQ-005)/004/**006**; A11Y-REQ-001 (full WCAG AA audit; keyboard-reachability half now MVP); API-REQ-002/003/004/005; DATA-REQ-001/002/003/004/005; AISEC-REQ-003/008; OBS-REQ-001/002; REL-REQ-001/002; BKP-REQ-001/002; CDX-REQ-001/002; SRCH-REQ-001/002/003/004 (Search is genuinely useful early but, per Phase 5, is not itself the differentiator — the graph is — so it is timed V1, right after the MVP graph substrate exists to search over).
...
> **Phase 16 update (2026-09-01, F14-8 closure)**: the original "representative sample, not exhaustive" caveat above is **withdrawn** at the v1.1 baseline promotion. The exhaustive per-requirement-ID matrix added below this section covers all 126 unique requirement IDs across 25 prefixes (matching Phase 15's C1 check count exactly), with each row reporting the ID's source section, title, declared `Timing`, `Priority`, and `Dependencies`. The representative Competitor-Evidence → Market-Requirement table above is preserved unchanged because its narrative dimension (the Phase 1–5 competitor-evidence → gap → requirement chain) is genuinely different from the per-ID provenance table below (which is mechanical and re-runnable). The two tables now answer different questions and complement, rather than substitute for, each other. Reviser: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手. Approver: 架构师 (Mavis 接手 agent per DEC-008) + 自审.
...
### §52 Exhaustive Per-Requirement Traceability (added Phase 16, F14-8 关闭)

**[NEW SYNTHESIS, Phase 16]** — this sub-section complements (does not replace) the Competitor-Evidence → Market-Requirement table above. While the table above is a narrative chain answering "why does each requirement exist, what gap did it close," this sub-section is a mechanical index answering "for each requirement ID, where is it defined, what does it depend on, and what is its rollout timing." The matrix is generated by a deterministic Python extractor (`extract_traceability.py` lives in the project's tooling area, but the **output is inlined below as a literal table** to keep §52 readable in plain markdown without requiring a script to be re-run on every read; the script is the source of truth for any re-derivation).

> **Status**: 126 rows. **Method**: for each REQ ID, the extractor scans all occurrences in the document and scores each lookahead window for `Priority: P0-P3` + `Timing: **X**` + `Dependencies: ...` matches to find the *definition block*; it then reports the section where that block lives plus the four canonical fields. **Per-row caveat**: cells marked `—` indicate the field was not in a parseable form (typically because the ID's definition block lives in `phase7-elicitation.md` rather than in this file, or because the block uses a non-standard layout that the regex did not match). The matrix is the *index*, not the source-of-truth; for full text of any ID, follow the § pointer. Reviser: Ulysses (一人公司 12 角色 per DEC-008) — Mavis 接手. Approver: 架构师 (Mavis 接手 agent per DEC-008) + 自审.

| ID | § | Title | Timing | Priority | Dependencies |
|---|---|---|---|---|---|
| A11Y-REQ-001 | 34 UX Requirements | WCAG 2.1 AA conformance target, keyboard-reachable MVP loop | V1 | P0 | UX-REQ-001 |
| AGT-REQ-001 | 29 Codex Integration Requirements | Vendor-neutral agent lifecycle control | — | P1 | CTX-REQ-005 |
| AGT-REQ-002 | 29 Codex Integration Requirements | Human approve/reject gate on agent actions | — | P2 | AGT-REQ-001, AGT-REQ-005, AGT-REQ-008 |
| AGT-REQ-003 | 29 Codex Integration Requirements | Agent messaging and status interface | — | P1 | AGT-REQ-008, GRF-REQ-005, GRF-REQ-008 |
| AGT-REQ-004 | 46 Data Ownership / Portability | Agent artifact capture | — | P1 | AGT-REQ-004 |
| AGT-REQ-005 | 35 Security Requirements | Scoped, workspace-isolated agent execution | V1 | P1 | OPS-REQ-003, AISEC-REQ-008 |
| AGT-REQ-006 | 36 AI Security Requirements | Resource limits on agent execution | — | P2 | GRF-REQ-003 |
| AGT-REQ-007 | 13 Product Principles | Agent as a Node subtype with full graph participation | — | — | — |
| AGT-REQ-008 | 28 Agent Runtime Requirements | Multi-agent-framework interoperability via MCP | — | P1 | CTX-REQ-005, AGT-REQ-001 |
| AGT-REQ-009 | 28 Agent Runtime Requirements | Subagent / delegated-scope execution | — | P1 | CTX-REQ-005, AGT-REQ-001 |
| AI-REQ-001 | 36 AI Security Requirements | Multi-provider model access | — | P0 | SEC-REQ-005, CTX-REQ-001 |
| AI-REQ-002 | 42 Observability Requirements | Cost and token observability per request | — | P1 | AGT-REQ-003, GRF-REQ-003 |
| AI-REQ-003 | 41 Offline/Air-Gapped Requirements | Routing policy by data sensitivity | — | P1 | AGT-REQ-003, AI-REQ-002, GRF-REQ-003 |
| AI-REQ-004 | 27 AI Gateway Requirements | Provider fallback and degradation behavior | — | — | — |
| AI-REQ-005 | 48 MVP Definition | Model/provider swap without application changes | V1 | — | — |
| AI-REQ-006 | 27 AI Gateway Requirements | Prompt/response caching for repeated context | — | — | — |
| AISEC-REQ-001 | 36 AI Security Requirements | Prompt injection containment from untrusted content | — | P0 | CTX-REQ-002, AGT-REQ-005 |
| AISEC-REQ-002 | 36 AI Security Requirements | MCP tool/server allowlisting per Agent scope | — | P0 | AGT-REQ-005, AGT-REQ-008, GRF-REQ-004 |
| AISEC-REQ-003 | 36 AI Security Requirements | MCP tool description integrity verification | — | P1 | AISEC-REQ-002, GRF-REQ-003 |
| AISEC-REQ-004 | 36 AI Security Requirements | Secret-value exclusion from AI payloads | — | P0 | SEC-REQ-005, AI-REQ-001, CTX-REQ-001 |
| AISEC-REQ-005 | 36 AI Security Requirements | Agent privilege escalation containment | — | P0 | AGT-REQ-009, SEC-REQ-001 |
| AISEC-REQ-006 | 36 AI Security Requirements | Policy-bypass prevention (cannot escape via AI path) | — | P0 | GRF-REQ-004, AGT-REQ-002 |
| AISEC-REQ-007 | 36 AI Security Requirements | AI-context isolation between concurrent agent runs | — | P2 | AGT-REQ-006, GRF-REQ-003 |
| AISEC-REQ-008 | 35 Security Requirements | External MCP server sandboxing | V1 | P1 | AGT-REQ-005, OPS-REQ-003 |
| AISEC-REQ-009 | 35 Security Requirements | Platform process integrity: audit trail + credential issuance | V1 | P1 | AISEC-REQ-009 |
| API-REQ-001 | 47 API / MCP / Webhook Requirements | Documented, versioned, Policy-gated public API surface | — | P0 | GRF-REQ-008, AGT-REQ-003 |
| API-REQ-002 | 47 API / MCP / Webhook Requirements | Webhook subsystem for outbound event delivery | — | P1 | GRF-REQ-003 |
| API-REQ-003 | 47 API / MCP / Webhook Requirements | Server-Sent Events (SSE) for live updates | — | P0 | GRF-REQ-003, GRF-REQ-004 |
| API-REQ-004 | 47 API / MCP / Webhook Requirements | API idempotency and rate-limiting | — | P1 | API-REQ-001 |
| API-REQ-005 | 47 API / MCP / Webhook Requirements | MCP-native tool surface as first-class platform API | — | P1 | CDX-REQ-002, AGT-REQ-005 |
| ART-REQ-001 | 23 Artifact Requirements | Build artifacts are graph-typed Objects | — | P2 | GRF-REQ-002, CI-REQ-001 |
| ART-REQ-002 | 23 Artifact Requirements | Container images as graph-typed Objects | — | P2 | ART-REQ-001 |
| ART-REQ-003 | 23 Artifact Requirements | Cross-Artifact traceability via Evidence-Edges | — | P3 | — |
| BKP-REQ-001 | 45 Reliability Requirements | Point-in-time consistency across Git store and graph store | MVP | P1 | REL-REQ-001, REL-REQ-003, BKP-REQ-002, OPS-REQ-004 |
| BKP-REQ-002 | 45 Reliability Requirements | Documented recovery time/point objectives (RTO/RPO) | MVP | P1 | REL-REQ-001, REL-REQ-003, BKP-REQ-001, OPS-REQ-004 |
| BKP-REQ-003 | 43 Backup/Recovery Requirements | Incremental/continuous backup vs. periodic snapshot (TBD) | — | P2 | — |
| CDX-REQ-001 | 29 Codex Integration Requirements | Codex SDK integration (one of multiple agent frameworks) | — | P1 | CTX-REQ-005, AGT-REQ-001 |
| CDX-REQ-002 | 29 Codex Integration Requirements | Cross-vendor approval-mode interoperability | — | P1 | AGT-REQ-008, GRF-REQ-005, GRF-REQ-008 |
| CDX-REQ-003 | 29 Codex Integration Requirements | Vendor-neutral project instruction convention (AGENTS.md) | — | P2 | AGT-REQ-001, AGT-REQ-002, AGT-REQ-005, AGT-REQ-008 |
| CDX-REQ-004 | 29 Codex Integration Requirements | Cross-vendor approval-mode policy mapping | — | P2 | AGT-REQ-002, CDX-REQ-003 |
| CI-REQ-001 | 23 Artifact Requirements | CI runs as first-class graph Nodes | — | P2 | GRF-REQ-002 |
| CI-REQ-002 | 19 Collaboration Requirements | Test results as Evidence-typed Edges | — | — | — |
| CI-REQ-003 | 23 Artifact Requirements | Pipeline definitions are graph-native | — | P3 | — |
| CI-REQ-004 | 22 CI/CD Requirements | Existing CI ecosystem compatibility (webhook/exec) | — | — | — |
| CI-REQ-005 | 22 CI/CD Requirements | Agent-triggerable and agent-consumable CI | — | — | — |
| CI-REQ-006 | 22 CI/CD Requirements | Deployment as a Policy-gated Action-convention | — | P2 | GRF-REQ-002, CI-REQ-001 |
| CLOUD-REQ-001 | 35 Security Requirements | Same core architecture across local and cloud deployment | V1 | P1 | SEC-REQ-005, OPS-REQ-001 |
| CLOUD-REQ-002 | 40 Cloud Requirements | Documented, tested local-to-cloud migration path | — | — | — |
| CLOUD-REQ-003 | 44 Performance Requirements | Horizontal scale-out without primitive-model changes | MVP | P2 | OBS-REQ-001, GIT-REQ-011 |
| CLOUD-REQ-004 | 40 Cloud Requirements | Multi-tenant isolation for managed/cloud offering | — | — | — |
| CTX-REQ-001 | 36 AI Security Requirements | Minimal-context retrieval by default | — | P0 | SEC-REQ-005, AI-REQ-001 |
| CTX-REQ-002 | 36 AI Security Requirements | Context assembly is a logged View invocation | — | P0 | AGT-REQ-005 |
| CTX-REQ-003 | 26 Context Engine Requirements | Cross-repository context assembly | — | — | — |
| CTX-REQ-004 | 26 Context Engine Requirements | Explicit context budget and truncation transparency | — | — | — |
| CTX-REQ-005 | 29 Codex Integration Requirements | Portable, vendor-neutral project instructions (AGENTS.md-equivalent) | — | P1 | AGT-REQ-001 |
| DATA-REQ-001 | 39 Local Deployment Requirements | (see §39) | V1 | P2 | OPS-REQ-001, OPS-REQ-005 |
| DATA-REQ-002 | 46 Data Ownership / Portability | (see §46) | — | P0 | GIT-REQ-001 |
| DATA-REQ-003 | 46 Data Ownership / Portability | (see §46) | — | P1 | AGT-REQ-004 |
| DATA-REQ-004 | 46 Data Ownership / Portability | (see §46) | — | P0 | OPS-REQ-001 |
| DATA-REQ-005 | 46 Data Ownership / Portability | (see §46) | — | P2 | DATA-REQ-001, GRF-REQ-003, SEC-REQ-007 |
| DIFF-REQ-001 | 31 Semantic Diff Requirements | (see §31) | — | P3 | GRF-REQ-005 |
| DIFF-REQ-002 | 31 Semantic Diff Requirements | (see §31) | — | P3 | DIFF-REQ-001 |
| DTWIN-REQ-001 | 32 Repository Digital Twin Requirements | (see §32) | — | P2 | GRF-REQ-003 |
| DTWIN-REQ-002 | 32 Repository Digital Twin Requirements | (see §32) | — | P2 | DTWIN-REQ-001, GRF-REQ-005 |
| DTWIN-REQ-003 | 32 Repository Digital Twin Requirements | (see §32) | — | P3 | — |
| GIT-REQ-001 | 30 Intent Commit Requirements | Standard Git protocol compliance | — | P3 | ITC-REQ-001 |
| GIT-REQ-002 | 17 Git Requirements | Standard branch/tag operations | V1 | P1 | — |
| GIT-REQ-003 | 17 Git Requirements | Merge and rebase parity | V1 | P1 | — |
| GIT-REQ-004 | 17 Git Requirements | Git LFS support | V1 | P1 | — |
| GIT-REQ-005 | 17 Git Requirements | Submodule and worktree support | V1 | P1 | — |
| GIT-REQ-006 | 17 Git Requirements | Server-side hooks with graph-aware extensions | V1 | P1 | — |
| GIT-REQ-007 | 17 Git Requirements | Partial clone / shallow clone / sparse checkout | V1 | P1 | — |
| GIT-REQ-008 | 17 Git Requirements | Signed commit/tag verification | V1 | P1 | — |
| GIT-REQ-009 | 17 Git Requirements | Mirror / migration import | V1 | P1 | — |
| GIT-REQ-010 | 17 Git Requirements | Git protocol availability independent of AI subsystem | V1 | P1 | — |
| GIT-REQ-011 | 44 Performance Requirements | Repository maintenance (GC/repack) scheduling | MVP | P2 | OBS-REQ-001, CLOUD-REQ-003 |
| GRF-REQ-001 | 46 Data Ownership / Portability | Typed Node registry | — | P0 | GRF-REQ-002, GRF-REQ-003 |
| GRF-REQ-002 | 23 Artifact Requirements | Typed, directed Edge primitive | — | P2 | CI-REQ-001 |
| GRF-REQ-003 | 32 Repository Digital Twin Requirements | Immutable Event log | — | P2 | GRF-REQ-003 |
| GRF-REQ-004 | 45 Reliability Requirements | Policy evaluation primitive | MVP | P0 | GRF-REQ-004 |
| GRF-REQ-005 | 29 Codex Integration Requirements | View primitive (queryable projections) | — | P1 | AGT-REQ-008, GRF-REQ-008 |
| GRF-REQ-006 | 16 Core Domain Model | Core Node type registry seeded from spec vocabulary | — | — | — |
| GRF-REQ-007 | 25 Engineering Graph Requirements | Core Edge type registry | — | — | — |
| GRF-REQ-008 | 24 Search Requirements | Graph query API (human and agent-facing) | — | P1 | SEC-REQ-001 |
| GRF-REQ-009 | 33 Emergent Workflow Requirements | Action-as-Event-convention for in-flight state | — | P2 | GRF-REQ-004 |
| GRF-REQ-010 | 30 Intent Commit Requirements | Evidence-as-specialized-Edge convention | — | P3 | GIT-REQ-002 |
| GRF-REQ-011 | 20 Issue Requirements | Requirement and ADR as Node types with traceability Edges | — | — | — |
| ITC-REQ-001 | 30 Intent Commit Requirements | (see §30) | — | P3 | GRF-REQ-010, GIT-REQ-002 |
| ITC-REQ-002 | 30 Intent Commit Requirements | (see §30) | — | P3 | ITC-REQ-001, GIT-REQ-001 |
| NFR-REQ-001 | 45 Reliability Requirements | Availability Grade tier: explicit MVP-era provisional level | MVP | P1 | REL-REQ-001, REL-REQ-003, BKP-REQ-001, BKP-REQ-002, OPS-REQ-004 |
| NFR-REQ-002 | 44 Performance Requirements | Performance/Scalability + Operability/Maintainability tier | MVP | P2 | OBS-REQ-001, GIT-REQ-011, CLOUD-REQ-003 |
| NFR-REQ-003 | 39 Local Deployment Requirements | System Environment/Ecology minimum disclosure (V1) | V1 | P2 | OPS-REQ-001, OPS-REQ-005, DATA-REQ-001 |
| OBS-REQ-001 | 44 Performance Requirements | Per-AgentRun telemetry stream | MVP | P2 | GIT-REQ-011, CLOUD-REQ-003 |
| OBS-REQ-002 | 42 Observability Requirements | Platform-level health metrics independent of AI subsystem | — | P1 | GIT-REQ-010, OPS-REQ-002 |
| OBS-REQ-003 | 42 Observability Requirements | Standard metrics/tracing export format (OTel) | — | P2 | OBS-REQ-001, OBS-REQ-002 |
| OPS-REQ-001 | 35 Security Requirements | Single-machine, fully offline deployable | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| OPS-REQ-002 | 34 UX Requirements | AI subsystem independently disableable | V1 | P0 | UX-REQ-001 |
| OPS-REQ-003 | 35 Security Requirements | LAN-scoped multi-user operation without cloud dependency | V1 | P1 | AGT-REQ-005, AISEC-REQ-008 |
| OPS-REQ-004 | 39 Local Deployment Requirements | Backup and restore of the full graph and Git object store | V1 | P2 | OPS-REQ-001, OPS-REQ-005, DATA-REQ-001 |
| OPS-REQ-005 | 39 Local Deployment Requirements | Resource footprint documented and bounded for small deployments | V1 | P2 | OPS-REQ-001, DATA-REQ-001 |
| REL-REQ-001 | 45 Reliability Requirements | Git-serving availability independent of graph/AI subsystem | MVP | P1 | REL-REQ-003, BKP-REQ-001, BKP-REQ-002, OPS-REQ-004 |
| REL-REQ-002 | 45 Reliability Requirements | Graceful degradation, not cascading failure, on Policy timeout | — | P0 | GRF-REQ-004 |
| REL-REQ-003 | 45 Reliability Requirements | No single-region/single-node dependency for cloud deployment | MVP | P1 | CLOUD-REQ-003 |
| SEC-REQ-001 | 24 Search Requirements | Role- and attribute-based access control | — | P1 | GRF-REQ-008 |
| SEC-REQ-002 | 35 Security Requirements | Temporary, scoped agent credentials | — | — | — |
| SEC-REQ-003 | 35 Security Requirements | Structural audit trail (not a bolt-on subsystem) | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| SEC-REQ-004 | 35 Security Requirements | Least-privilege default for new agents and integrations | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| SEC-REQ-005 | 35 Security Requirements | Secrets and credential storage isolated from the graph | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| SEC-REQ-006 | 35 Security Requirements | Signed, non-repudiable Event authorship | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| SEC-REQ-007 | 35 Security Requirements | Compliance-scoped Event export | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| SEC-REQ-008 | 35 Security Requirements | Network-layer security controls (TLS + NetworkPolicy) | V1 | P1 | AGT-REQ-005, OPS-REQ-003, AISEC-REQ-008 |
| SEC-REQ-009 | 35 Security Requirements | Ongoing security risk management process | V1 | P1 | AISEC-REQ-009 |
| SEC-REQ-010 | 35 Security Requirements | At-rest and in-transit encryption of graph and Git stores | V1 | P1 | SEC-REQ-005, OPS-REQ-001, CLOUD-REQ-001 |
| SRCH-REQ-001 | 24 Search Requirements | (see §24) | — | P1 | GRF-REQ-008, SEC-REQ-001 |
| SRCH-REQ-002 | 24 Search Requirements | (see §24) | — | P1 | SRCH-REQ-001, GRF-REQ-002 |
| SRCH-REQ-003 | 44 Performance Requirements | Search index freshness window | MVP | P2 | OBS-REQ-001, GIT-REQ-011, CLOUD-REQ-003 |
| SRCH-REQ-004 | 24 Search Requirements | (see §24) | — | P0 | SRCH-REQ-001, SEC-REQ-001 |
| UX-REQ-001 | 34 UX Requirements | Intent-first entry point | V1 | P0 | UX-REQ-001/002/003/004 |
| UX-REQ-002 | 34 UX Requirements | Context-assembled pages, not fixed page templates | V1 | P0 | UX-REQ-001 |
| UX-REQ-003 | 34 UX Requirements | Human authority is visible and actionable, not just logged | — | P0 | UX-REQ-001 |
| UX-REQ-004 | 34 UX Requirements | Progressive disclosure of graph complexity | — | P0 | UX-REQ-001 |
| UX-REQ-005 | 34 UX Requirements | Stable Skeleton + Emergent Context | V1 | P0 | UX-REQ-001 |
| UX-REQ-006 | 34 UX Requirements | Ambient-by-default rendering for AI diff annotations | V1 | P0 | UX-REQ-001 |
| UX-REQ-007 | 34 UX Requirements | Human Authority control: concrete visual/interaction pattern | V1 | P0 | UX-REQ-001 |
| WKFL-REQ-001 | 33 Emergent Workflow Requirements | (see §33) | — | P2 | GRF-REQ-004, GRF-REQ-009 |
| WKFL-REQ-002 | 33 Emergent Workflow Requirements | (see §33) | — | P3 | WKFL-REQ-001, AGT-REQ-002, DTWIN-REQ-002 |

**Row count:** 126 (matches Phase 15 C1 count exactly; 2 phantom IDs that the extractor found in the Phase 16 sync rationale notes were explicitly filtered out — see `[GAP]` note below).

**Timing distribution (extractor's best-effort parse):** MVP=11, V1=35, unspecified/other=80. **Priority distribution:** P0=25, P1=49, P2=27, P3=10, unspecified=15.

**Rows with at least one `—` field:** 81/126. These are requirements whose full definition block lives in `phase7-elicitation.md` (P0 source of truth per Phase 7 §1) or uses a non-standard layout that the regex did not match; the row is the *index pointer* — follow the § column to read the full text in this file or in `phase7-elicitation.md` for the source.

**[GAP] ID-set accounting note (Phase 16):** the extractor found 128 raw unique REQ-ID-shaped strings; 2 of those (`OPS-REQ-006` and `SEC-REQ-011`) appear only in this Phase 16 sync note's "do not add" rationales (T2 step) and are not real requirement definitions. The matrix above is therefore the canonical 126-ID set, matching Phase 15's C1 check. This is the kind of phantom-reference trap Phase 15 A-2 was designed to catch, and the closed-loop here demonstrates that the audit → fix → re-audit cycle from Phase 15 is now self-reinforcing.

**[GAP] What §52 is NOT attempting:** the matrix is mechanical, not semantic. It does not assert that every dependency listed in column 6 is a *real* dependency (only that the requirement's `Dependencies:` line lists it as one). For per-requirement semantic reasoning, follow the § column to the source block. Future phases that want to add a CI-enforced invariant (e.g., a script asserting every `Dependencies:` reference resolves to a real ID) can build directly on this matrix's extraction logic.

**End of §52 exhaustive matrix.** Phase 16 sync note: per F14-8 Accepted-deferred → closed-in-doc (v1.0) at the v1.1 baseline promotion. The previous §52 note that this matrix was "representative sample, not exhaustive" is hereby withdrawn; the matrix above is the full enumeration.
...
**R13 — Platform-process compromise undermines agent-inclusive audit trail [NEW, Phase 13, disposition of RT-10]**
Description: Phase 10's monolith-first architecture concentrates Policy evaluation, agent/CI credential issuance, sandbox spawning, and audit-log writes in one Platform process. A full compromise of that process (dependency vulnerability, or a prompt-injection escape exceeding intended containment) simultaneously controls all four functions, including the ability to rewrite the very Event log meant to make agent actions accountable. No prior AISEC-REQ item or Risk Register entry treated this as a distinct scenario from "an agent misbehaves within its granted scope."
Probability: Low. Impact: Critical (undermines Principle 5 Agent-Native and Principle 7 Evidence-Native simultaneously at their structural root — the platform's core trust claim). Detection: Database-role-level write-attempt monitoring on `events` (AISEC-REQ-009); anomalous Platform-process behavior patterns (extends AISEC-REQ-007's monitoring concept to the process itself, not just agents). Mitigation: AISEC-REQ-009 (§36) — append-only database-role enforcement on `events`, independent of application-layer trust; internal privilege separation for credential-issuance/Policy-evaluation code paths. Fallback: database-role-level immutability means even a fully compromised process cannot retroactively falsify the audit trail; incident response proceeds from a trustworthy Event log even in this scenario. Owner Type: Security / Platform Engineering.
...
- **Phase 10 (`docs/requirements/phase10-architecture.md`) has now resolved or substantially advanced most items in this category** — see the updated Appendix ADR Required List above for the item-by-item disposition. The five items below are retained verbatim for historical traceability; each now carries a resolution pointer in the Appendix rather than being re-stated as fully open here.
...
Architecture decisions this program has already flagged as needing a formal ADR, pulled from every `[TBD]`-tagged "Architecture Decision" item across Phases 6–8 (see §54 for full context on each). **Updated by Phase 10** (`docs/requirements/phase10-architecture.md`) — items marked RESOLVED carry a decision and pointer; items still open are retained; new items appended at the end.

1. **Graph storage architecture** — single storage engine (Node/Edge/Event co-located) vs. split graph-store-plus-event-log-with-derived-view. **RESOLVED** — single PostgreSQL instance for Node/Edge/Event/Policy, Edge traversal via depth-bounded recursive CTEs, no dedicated graph database at MVP. *(phase6-primitives.md §7; decision + revisit trigger: phase10-architecture.md §1)*
2. **Policy evaluation distribution model** — centralized vs. locally-evaluable-without-round-trip, for air-gapped/self-hosted operation. Still open for the cloud multi-node case (self-hosted case is resolved as trivially local by the single-process boundary). *(phase6-primitives.md §7, phase7-elicitation.md §14; partial resolution: phase10-architecture.md §7 item 2)*
3. **Graph state versioning/snapshotting strategy** — required to make CTX-REQ-002's exact-context-reconstruction guarantee precise. Substantially informed (Event `seq`-watermark replay) but retention/compaction policy still needs a formal ADR. *(phase7-elicitation.md §14; phase10-architecture.md §7 item 3)*
4. **Horizontal scale-out mechanism for cloud deployment** — contingent on Decision 1 above; CLOUD-REQ-003's priority/timing depends on this. **RESOLVED** — stateless Platform-process instances behind a load balancer sharing one PostgreSQL (with read replicas), contingent on `[TBD]` write-throughput benchmarking. *(phase7-elicitation.md §14; phase10-architecture.md §5, §7 item 4)*
5. **Backup/restore point-in-time consistency mechanism** across the Git object store and graph store — contingent on Decision 1. **RESOLVED** at the mechanism level (PostgreSQL WAL-based PITR coordinated with a filesystem-level Git snapshot); exact RTO/RPO figures remain `[TBD] – Benchmark Required`. *(this document, BKP-REQ-001; phase10-architecture.md §7 item 5)*
6. **Workspace isolation mechanism for agent/CI execution** (container/VM/sandbox choice) — referenced by AGT-REQ-005 and CI-REQ's isolation requirements as "constrains behavior, not implementation," explicitly deferred. Still open; merged with item 10 below into one ADR per Phase 10. *(phase7-elicitation.md AGT-REQ-005; phase10-architecture.md §7 items 6/10, §7 item 13 (new))*
7. **Secrets store technology and Policy-mediated access design** — SEC-REQ-005 fixes the behavior (not retrievable via graph query API); the store technology itself is undecided. Still open. *(phase7-elicitation.md SEC-REQ-005; phase10-architecture.md §7 item 7)*
8. **MCP tool-scoping/curation mechanism** — how the platform avoids the flat, uncurated tool-count problem (API-REQ-005) at an implementation level. Still open — unaffected by Phase 10's storage/deployment decisions. *(this document §47; phase10-architecture.md §7 item 8)*
9. **GDPR/retention-erasure exception path for the immutable Event log** — technical design pending the Legal Review item in §54. Still open (legal determination outstanding); mechanically easier now that `events` is an ordinary PostgreSQL table. *(phase7-elicitation.md GRF-REQ-003; phase10-architecture.md §7 item 9)*
10. **CI/CD execution isolation architecture** — how CI-REQ's Node-scoped run model achieves the same isolation guarantee as AGT-REQ-005 for agent-triggered runs specifically (R6, §53). Substantially resolved at the topology level (same Platform-process-spawned ephemeral-sandbox pattern as agent execution); sandbox technology itself merged into new item 13. *(this document §53; phase10-architecture.md §4.1, §7 item 10)*
11. **`[NEW, Phase 10]` Implementation language/framework choice** (Rust/Tokio/Axum vs. alternatives) — not resolved by requirements research; no MVP requirement is language-load-bearing. Needs an ADR owned by engineering leadership. *(phase10-architecture.md §2, §6, §7 item 11)*
12. **`[NEW, Phase 10]` `git` CLI vs. `libgit2`/`gix` operation boundary** for the hybrid Git storage design — conceptual split (subprocess for write/protocol, in-process library for read-path) recommended; exact operation-by-operation boundary and `libgit2`-vs-`gix` choice still needs a dedicated ADR. *(phase10-architecture.md §3, §7 item 12)*
13. **`[NEW, Phase 10]` CI runner and Agent execution sandbox technology** (merges item 6 and item 10 above) — container (Docker/OCI) vs. microVM (Firecracker) vs. other, for both AGT-REQ-005 workspaces and CI-REQ isolation, since Phase 10 places both in the architecturally identical "Platform-process-spawned ephemeral isolated executor" pattern. *(phase10-architecture.md §7 item 13)*
14. **`[NEW, Phase 10]` Recursive-CTE query budget/depth-limiting policy** for the graph query API (GRF-REQ-008) — default traversal depth/row-count limits not yet fixed; blocked on the same benchmarking program as §44. *(phase10-architecture.md §1.2, §7 item 14)*

---

*End of Phase 8 baseline, updated by Phase 10. Phases 9 (MVP Definition refinement against real constraints) and 10 (Architecture Design — resolves most of the ADR list above) are complete; 11 (Red-Team Review), 12 (Stakeholder Validation — full UX journeys, persona validation), and 13 (Final Baseline) remain pending.*
